...
export default function PropertyOwnersPage() {
  ...
  const { data: links = [], isLoading } = useQuery<LinkItem[]>({
    queryKey: ["/links", "PROPERTY_OWNER"],
    queryFn: () => api.get("/links?targetRole=PROPERTY_OWNER"),
  });

  const inviteMutation = useMutation({
    mutationFn: () => api.post("/links/invite", { email: email.trim(), targetRole: "PROPERTY_OWNER" }),
    onSuccess: () => {
      toast({ title: "Invite sent", description: "Property Owner will be notified" });
      setShowInvite(false);
      setEmail("");
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage your linked Property Owners</p>
        </div>
      ...
      {/* Linked Property Owners */}
      <div>
        <h2 className="text-sm font-medium text-muted-foreground mb-3">
          Linked Property Owners ({accepted.length})
        </h2>
        {isLoading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">No linked Property Owners yet</p>
      ...
      <Dialog open={showInvite} onOpenChange={setShowInvite}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Enter the email of the Property Owner you want to link with.</DialogDescription>
          </DialogHeader>
          <form
            onSubmit={(e) => {
              e.preventDefault();
