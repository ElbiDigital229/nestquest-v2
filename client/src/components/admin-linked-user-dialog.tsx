...
import { kycBadgeProps, ROLE_LABELS } from "@/lib/role-utils";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function AdminLinkedUserDialog({
  guestId,
  open,
  onClose,
}: {
    ...
    <Dialog open={open} onOpenChange={(isOpen) => { if (!isOpen) onClose(); }}>
      <DialogContent className="max-w-lg max-h-[85vh] p-0">
        <DialogHeader className="px-6 pt-6 pb-0">
          <DialogTitle>Linked User Details</DialogTitle>
          <DialogDescription>Read-only profile view</DialogDescription>
        </DialogHeader>
          ...
          <ScrollArea className="max-h-[70vh] px-6 pb-6">
            <div className="space-y-5 pb-4">
              ...
              <div>
                ...
                <InfoRow icon={Calendar} label="Member Since" value={formatDate(u.createdAt)} />
              </div>
            </div>
          </ScrollArea>
        )}
